...
valiSetPath = './DataSet/ValidationSet.tfrecords'
testSetPath = './DataSet/TestSet.tfrecords'
test_img, test_label = loadData(valiSetPath)
test_img_batch, test_label_batch = tf.train.batch([test_img,test_label],
                                                batch_size=batch_size,capacity=3589)
...
model_path = "./Saver/model_parameters_final.ckpt"
...
for step in range(10):
    ...
    p = sess.run([top_k_op], feed_dict={image_holder: image_batch, label_holder: label_batch})  # 计算精度
    # 这里的精度，也就是top_k_op，是指这批样本中预测类别和真实类别相等的情况
    # 返回的是一个矩阵［1,1,0,0...1,0,1］，为0则代表预测错误，矩阵维度等于批处理样本的数量

    true_count += np.sum(p)  # 计算这批样本中预测正确的个数

precision = true_count / (10 * batch_size) * 100  # 计算精度（准确率）
